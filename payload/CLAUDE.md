...
- **An unquoted `$var` is NOT word-split in zsh**, unlike bash, so a list built into a variable reaches a command as ONE argument. `FILES=$(grep -rl ...)` then `git log -- $FILES` passes a single newline-laden path, matches nothing, and prints a clean zero: the command succeeds and the answer reads as a finding. On 2026-09-23 that returned "0 model shape changes in 60 days" when the real answer was 110, and a design was nearly built on it. Command substitution IS split (`git log -- $(cat list.txt)` works), which is why the two forms disagree. Use `$(...)` directly, or a real array (`files=(...); cmd -- $files`), and sanity-check any zero against one case you already know the answer to.
...
- **Anything started in the background is yours to stop.** A dev server, a poll loop, a watcher: before ending a turn that started one, kill it and say so. A backgrounded command reports nothing while it runs, so a broken one is indistinguishable from a working one and outlives the session (2026-09-22: an `until curl ...; do sleep 3; done` whose URL was unquoted, so zsh failed on the `?` and curl never ran at all, spun for two and a half hours writing 183KB of the same error and was found only because Dan asked what was running).
- **Never cluster `rg`'s `-r` with other short flags.** `-r` takes a REPLACEMENT value, so `rg -rl`, `rg -rn` and `rg -ril` consume the following letters as that replacement and print every match REWRITTEN to it. The output reads as ordinary source, so an identifier taken from it can be one that exists nowhere (2026-09-21: `case sameNight([Night])` came back as `case ln([Night])`). Write short flags separately (`rg -l -n`), and re-derive any symbol or line reference from a plain `grep -n` or a direct read before acting on it.
...
- The `check-style-guide.sh` pre-push hook rejects the ENTIRE Bash call when it sees `git push` anywhere in it, before any earlier command in the chain runs. So never chain `... && git commit --amend && git push`: the amend silently never happens and the hook re-reads the old commit. Run the fix, the amend, and the push as separate Bash calls. **The damage is usually done by the NEXT command, not by the blocked one** (Slate, 2026-09-22). A chained FIRST commit that never ran leaves the branch tip still on whatever it was cut from, so a `git commit --amend` afterwards rewrites THAT commit, which on a fresh branch is the upstream tip somebody else already merged: the branch then reports as conflicting and the fix is `git reset --soft origin/main`, not a rebase. Before any amend, confirm `git log -1` is your own commit.
...
- **Subagents never drive the Playwright browser.** The Playwright MCP server is one browser for the whole session, so a parallel agent can navigate it mid read and an evaluate returns the sibling's page as your answer (three of five research agents hit this on 2026-09-14). A subagent reads pages with WebFetch, or with the Claude in Chrome tools in a tab it created; the main session keeps Playwright. `hooks/playwright-subagent-gate.sh` refuses any Playwright call whose hook payload carries an `agent_id`, which only a subagent's does.
- **Each subagent keeps its own scratchpad subdirectory.** Every agent a session dispatches is handed the same scratchpad, so on 2026-09-21 one agent's `pr-body.md` was overwritten by a sibling's between its write and `gh pr create`. Tell each agent to write its working files under `scratchpad/<its issue number or task name>/`. `hooks/scratchpad-subagent-gate.sh` refuses a subagent's write straight into the scratchpad root and names the path to use instead (claude-config#527).
...
- **In a worktree, judge git and gh by the state they leave, never by what they print.** Worktrees share refs, so once another worktree has fetched, your `git fetch` prints no update line however far main has moved (on 2026-09-14 that let an Ovation PR merge without the rebase its brief required); after fetching, `git rev-list --count HEAD..origin/main` is how many commits you are behind. And `gh pr merge --squash --delete-branch` from a detached HEAD merges and then fails with "could not determine current branch", which reads as a failed merge and leaves the remote branch behind: confirm with `gh pr view <number> --json state`, then delete the branch with `gh api -X DELETE repos/<owner>/<repo>/git/refs/heads/<branch>`.
