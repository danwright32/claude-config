...
- **Every issue gets a milestone, always.** Before filing any issue, read the repo's open milestones and attach the one that fits. If an open milestone fits, attach it and say which one, no question needed. If none fits, never invent one silently: propose the new milestone in the same picker that approves the issue, then resolve it through `~/.claude/skills/milestone/ensure-milestone.sh` (it reuses a match, refuses to create a near duplicate of an open milestone, and only creates with `--create-approved`). Use the exact title it reports, since `gh` matches milestones by name, not number. A PreToolUse gate (`require-milestone-on-issue.sh`) blocks any `gh issue create` without `--milestone`; the visible override for a repo where milestones genuinely do not apply is `SKIP_MILESTONE_CHECK=1 <the same command>`, explained to the user first, never silently.
