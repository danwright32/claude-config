...
#
# The scan far below is python3 and nothing here asked whether python3 was installed. Without it
# the scan returned an empty string, both readings came back with no findings, and this gate exited
# 0: on such a machine EVERY push read as style clean, with nothing said. A detector that cannot
# run finds nothing, and finding nothing is exactly what a clean run looks like (L42, L98).
#
# Asked here, above the payload read, rather than beside the scan: python3 is also one of the two
# tools ps_parse_payload reads the payload with, so on a machine with neither jq nor python3 the
# parse below exits 0 first and the question would never be reached (L135, L667).
#
# Nothing here can tell a push from an `ls` when the payload is unreadable, so what is refused is
# narrowed by a cheap substring on the raw payload: a command with no "push" anywhere in it cannot
# be one. Refusing every Bash command would be a gate nobody keeps (L36, L54). The override is read
# the same way, so the refusal is never a dead end (L109).
if ps_reader_missing python3; then
  case "$payload" in
    *SKIP_STYLE_CHECK=1*) exit 0 ;;
    *push*)
      echo "PUSH BLOCKED: $(ps_detector_absent_why "python3 is not on PATH" \
        "check-style-guide.sh scans the lines this push adds for an em dash, an en dash or an emoji with it, so with python3 absent nothing here reads the diff at all." \
        "python3")" >&2
      echo "OVERRIDE, this one command: SKIP_STYLE_CHECK=1 <your original git push command>" >&2
      exit 2 ;;
  esac
  exit 0
fi
