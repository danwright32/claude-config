...
reason = (
    ...
    "they choose.\n\n"
    "4. If this is a standalone bug or chore that belongs to no feature, put it in the repo's catch-all "
    "milestone, \"Ungrouped\". The helper creates that one without needing approval:\n"
    "   bash ~/.claude/skills/milestone/ensure-milestone.sh \"<owner>/<name>\" \"Ungrouped\"\n\n"
    "A milestone is the overarching FEATURE, and its issues are what has to ship for that feature to be "
    "done, so its title NAMES the feature, carries NO punctuation at all (a comma or colon is refused at any "
    "length, because a name has no clauses) and runs to at most 8 words: Saved views, Salesforce sync v2, "
    "Bulk contact enrichment for scouted shows. Never a narrative sentence, and never a category like Accessibility (categories are "
    "labels, because an issue can be two at once). The full rule is in "
