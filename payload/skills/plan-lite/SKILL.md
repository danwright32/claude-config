...
### 1. Frame by grilling  (user)
Confirm the basics first: the feature (a sentence or two), the **project directory**, and any hard constraints or success criteria.

**Then, before the first grilling question, find any earlier verdict on this approach.** Run `bash ~/.claude/skills/plan-council/find-prior-verdicts.sh --dir <project dir> "<term>" "<term>"` with two to four of the approach's distinctive phrases (the mechanism, not the feature's generic name). It searches the project's decision records (DESIGN, README, CLAUDE, AGENTS, ADR and docs markdown) and every issue, closed ones included. Quote each earlier verdict to the user **with its date** and where it is recorded, and ask whether it still stands before building on it; a decision is only true as of its date (L61). When it prints `NONE FOUND`, say so in one line, naming what was searched. When it prints `COULD NOT SEARCH`, say which source went unsearched: an unsearched source is not an empty one. Run it again with the draft's own words in step 2 if the draft settles on a mechanism the first search did not name.

Then take a quick read of the repo and **invoke the `grilling` skill** (`Skill` tool, skill name `grilling`) to pressure-test the framing before you draft: **one question at a time**, waiting for each answer, every question carrying YOUR recommended answer, asked through **AskUserQuestion**, and anything the code can answer gets answered by reading the code instead of being asked. Invoke `grilling`, NOT `grill-me`: `grill-me` is a user-typed command and you cannot call it.
...
1. **Red-team**, whose only job is to attack the draft: "Here is a plan for <feature>. Try to find its real failure modes, wrong assumptions about the codebase, missing edge cases, and hidden costs. Be skeptical; assume it has at least one serious flaw and find it." Pass it the draft plan and enough context to ground itself.
2. **Lessons auditor**: first run `ls ~/.claude/LESSONS-INDEX-*.md` yourself and hand it that list. Tell it to read every index file on the list in full (one shortened line per lesson, about 105k chars in all) plus the build-time rules in `~/.claude/CLAUDE.md`, and NOT to read `~/.claude/LESSONS.md`, which at over 860k chars cannot be read whole (claude-config#561). Because an index line is a shortened rule, it reads the full entry of any lesson a finding relies on with `~/claude-config-sync/claude-sync lesson Lnnn` before citing it. Then it reports every place the plan repeats a mistake already recorded there, or plainly ignores a lesson that applies (a background job with no alert on failure or on a missing run, a guard that fails open, a multi-step write that breaks if it runs twice, a route with no stated caller and no tenant scoping, a destructive step with no backup or undo, an error rendered as an empty state, a test that cannot fail). Each finding: the lesson id, the exact part of the plan at fault, why it is the same mistake, and the concrete fix. Require it to name every index file it read: compare that against the list you gave it, and an audit that skipped a file or could not read them reports **"could not audit"**, never "clean".
...
- **Lessons audit next:** any violation you did NOT fix gets led with, by lesson id, along with why you pushed back. If the auditor could not read every lessons index file, say plainly that the plan was never checked against them, because unaudited is not clean and must never be reported as "no issues found". If it came back clean, one line saying so is enough.
