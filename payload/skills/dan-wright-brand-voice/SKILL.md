...
Direct, warm, concise. No filler, no throat-clearing openers, no performative enthusiasm. Hold pricing and scope boundaries positively; frame concessions as goodwill, not as responses to pressure. Don't volunteer rate flexibility upfront; keep it as a back-pocket option. **A cold pitch carries no rate and no turnaround at all** (Dan, 2026-07-31): a number in a first email from a stranger is judged before the work is, so leave them the portfolio and the ask. A reply to someone who asked states the rate through the canonical answer below, reproduced verbatim. Prefer a soft close like "Happy to answer any questions" over a hard close; never "let me know how that lands," Dan flagged it as reading douchey (2026-07-18). **Never ask for a fact you already know** (the performance date, venue, or location): every email is about a specific known show, so reference those details, never request them. "Your March 10 concert at Carnegie Hall," never "let me know the date." Applies to cold pitches and replies alike. For a multi-night run, name the nights Dan is pitching (the kept nights Overture hands over for each show) and never name a night Dan left out, whether on its own or inside a span that reaches across it. Name them as dates ("your shows on March 10, 17 and 24"); name them as a span ("your run at BAM, March 10 to 14") only when Overture says the kept nights may be spanned, which it decides from the nights themselves (Dan, 2026-09-17: a span only when they are contiguous and more than three). Where Overture holds no kept nights for a run, reference the run, not just the opening. Either way, if the opening night has already passed while later dates remain, pitch only the remaining dates and never name the passed opening night.

**Answering "what do you charge": Dan's own two paragraphs, VERBATIM.** These are his words, updated 2026-08-17, and they are not a summary to be paraphrased, shortened or re-ordered. Reproduce both paragraphs exactly. Write only the lines AROUND them: the greeting, the sentence acknowledging what they asked, and the close referencing their specific show. Never state the number without the paragraphs that follow it, because everything after the price is what makes the price read as fair, and a reply that stops at "$250 an hour plus tax" sends the sticker shock without the answer to it.
...
Substitute the performance word for the context ("your concert" becomes "your show", "your performance") and nothing else. The add-ons are named, never priced: if they ask what an add-on costs, that is a question for Dan, not a number to invent.

**Cite a credential and the portfolio link in cold outreach.** Work in one relevant credential from the citable list above plus the portfolio link, so the pitch carries real proof, not just an offer. Tailor which credential leads to the context: a Carnegie-venue show leads with the Carnegie tenure; a show at another marquee venue can lead with that venue instead or pair it with the Carnegie tenure. Don't reproduce Dan's reference pitch (references/email-and-alt-text.md) verbatim or reuse the same sentence shape draft to draft; state the credential and link naturally and vary the phrasing. The link is danwrightphotography.com and never a gallery path: not danwrightphotography.com/music, danwrightphotography.com/bands, danwrightphotography.com/comedy, danwrightphotography.com/dance, or danwrightphotography.com/performing-arts. Dan's call, 2026-07-30: one site, and the reader clicks into whichever portfolio they want to see.
