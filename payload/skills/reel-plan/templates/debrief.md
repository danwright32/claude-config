# Debrief: {EVENT}

Filled in after the shoot, before the edit. The point is that the next plan is better than this one.

---

## Planned versus captured

| Shot | Planned | Got it? | Why not |
|---|---|---|---|
| | | | |

**Safety shots banked:** {N of N}
**Ambitious shots attempted:** {N} **Landed:** {N}

---

## What the plan got wrong

- **Predicted and did not happen:** {}
- **Happened and was not predicted:** {}
- **The story spine was:** {right / wrong, and the real story was ...}

## Structure

- **Chosen:** {} **Did it hold?** {}
- **Does this reel look like the last one?** Be honest. {}
- **Structures now used, most recent first:** {list, so the next plan can avoid them}

---

## What was physically impossible

Shots the plan asked for that the room, the light, or the rig would not allow. These are the plan's fault, not the shoot's.

- {}

---

## Audio

- **Tier expected:** {} **Tier actually got:** {}
- **Sync usable?** {}
- **Anything unpostable?** {}

---

## Discovered on the day

- Anything about the room, the light, the sound, or the running order that should have been known in advance: {}

---

## The one change to the next plan

> {single sentence}

---

## For the skill

Anything here that is not specific to this event, and should be written into the reference files: {}
