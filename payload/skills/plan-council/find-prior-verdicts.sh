...
while [ $# -gt 0 ]; do
  case "$1" in
    # A flag given last has no value, and `shift 2` with one argument left fails WITHOUT shifting,
    # which looped this parser for ever (found by the first real PR lessons review, 2026-09-24).
    --dir|--repo)
      [ $# -ge 2 ] || { echo "find-prior-verdicts: $1 needs a value." >&2; exit 64; }
      if [ "$1" = "--dir" ]; then dir="$2"; else repo="$2"; fi
      shift 2 ;;
