...
LESSONS.md, which is NOT loaded into the session. 342 lessons.
...
- L317. A pipeline change is judged first by the count of tests it executed against the count expected, before a single failure is read, and the readout is built BEFORE the change that could produce a partial run, because a partial run that reports a verdict is worse than a failed run (L288, L98).
