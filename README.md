...
| `.resurrected-reported` | a send that refused to publish a leftover copy back over a deletion | that same refusal, to say it once per file rather than on every edit | absent means nothing has been refused here, which is the normal state. Each line is the path AND a digest of its content, so a file that CHANGES is a new decision and is reported again, and a file the person deletes or edits stops matching. Losing it costs one repeated message and nothing else, which is why it is not treated as important state |
...
| `.behind-skips` | a watcher send that found itself behind and whose reconcile did not clear it | `claude-sync status`, and the escalation that tells you once rather than on every edit | absent means sending is not stuck, which is the normal state and is said by saying nothing. It holds the count, the moment the run of skips started, and the moment of the last one; any run that gets through removes it, so the count is a measurement of the current state rather than a total that only grows. A count that will not parse reads as zero, which errs toward not accusing |
