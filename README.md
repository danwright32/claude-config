...
| Setting | Default | What it does |
| --- | --- | --- |
| `SUITE_TIMEOUT` | `3600` | Seconds of wall clock before a run is killed however well it is going. Since #152 this is only an absolute ceiling for a runaway, not what catches a hang: `SUITE_STALL_TIMEOUT` does that. Generous on purpose, because a full single process run measured 348 seconds idle and 1943 at load 160 to 188 on 2026-08-22, so a tighter one kills a healthy run on a busy Mac. `0` disables it. |
| `SUITE_STALL_TIMEOUT` | `600` | Seconds a run may go without reaching a new section before it is killed as hung and told which section it stopped in. This is what actually catches a hang, and a machine that is merely slow keeps moving between sections and is left alone. The slowest single section measured 34 seconds on 2026-08-22, so this is at least 3x the worst observed, and the suite checks that ratio against the sections the run really took rather than against this sentence. `0` disables it. Both this and `SUITE_TIMEOUT` at `0` is refused, since that is a run with no bound at all. |
...
| `SYNC_SCRATCH_MAX_AGE` | `3600` | Seconds before scratch counts as abandoned. A suite run cannot outlive its own ceiling, which is also 1 hour since #152, and a sync took 6 seconds when it was timed on 2026-08-17, so this is 600x the sync. The cost is that a burst of interrupted runs is not reclaimed until an hour after the last of them. `0` turns the sweep off entirely, and a value that is not a whole number is refused rather than guessed at. |
